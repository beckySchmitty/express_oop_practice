...
class Cat {
  ...
  static async create(name, age) {
    if (!name || !age) {
      throw new ExpressError("Missing required data. Must have name and age", 400)
    }
    const result = await db.query(`INSERT INTO cats
     (name, age) VALUES ($1, $2) 
...
static async delete(id) {
  const result = await db.query(`DELETE FROM cats WHERE id=$1 RETURNING id`, [id])
  if (result.rows.length === 0) {
